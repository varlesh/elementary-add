# Credits

This list contains references to the authors and original sources of the icons contained in this theme.

Alexey Varfolomeev (varlesh@gmail.com)
apps
* acestreamplayer
* acetoneiso
* alarm-clock
* amarok
* amd
* application-x-clementine
* aptik
* ardour (Source Numix Circle)
* atlas
* audacious
* audacity
* backintime-gnome
* baobab
* battleblock-theater (Source Numix Circle)
* bindingofisaac
* birdie
* bitwig-studio
* bleachbit
* blender
* bluefish
* bomi
* brackets
* brasero
* browser-tor
* cheese
* cherrytree
* chess
* clamav
* corebird
* darktable
* dconf-editor
* deadbeef
* deluge
* display.im6
* distance
* dosbox
* doublecmd
* dukto
* easytag
* eclipse
* eurotruck-simulator-2
* everlasting-summer
* evernote (Source Numix Circle)
* evolution
* filezilla
* firefox
* foobnix
* gajim
* gconf-editor
* gdebi
* geany
* ghex
* gimp
* gloobus-preview
* gmtp
* gnome-disks
* gnome-mplayer
* gnome-mpv
* google-chrome
* google-chrome2
* GP6-icon
* gparted
* gprename
* grid-autosport
* grub-customizer
* gtkpod
* guake
* gufw
* gwoffice
* gzdoom
* hb-icon
* homebank
* hotline-miami
* hotline-miami2
* htop
* i-nex
* inkscape
* intel-installer
* intellij
* jack_mixer
* java
* kdeconnect
* kdenlive
* keepassx
* kodi
* librecad
* lightworks  (Source Numix Circle)
* limbo
* line6
* lingot-icon
* linphone
* lmms
* maxthon-browser
* mc
* mega
* meld
* minitube
* mixxx
* modem-manager-gui
* motorbike
* mpv
* musique
* my-weather-indicator
* nvidia
* oceanaudio
* openshot
* opera
* paromplayer
* photoshop
* pidgin
* pillarsofeternity
* pinta
* pitivi
* playonlinux  (Source Numix Circle)
* pomodoro
* popcorntime
* ppsspp
* psensor
* pycharm
* qbittorrent
* qmmp
* qtcreator
* radiotray
* rakarrack
* rhythmbox
* risk-of-rain
* robocraft
* rosegarden
* scribus
* shutter (Source Numix Circle)
* skype
* smartgit
* smplayer
* soundconverter
* springseed
* squeegee
* starbound
* steam
* steam_icon_239140
* steam_icon_250900
* steam_icon_251570
* steam_icon_255710
* steam_icon_295270
* stellarium
* stuntrally
* sublime-text
* sunflower
* supermeatboy
* supertuxkart
* synapse
* synaptic
* teamviewer
* telegram
* terraria
* tf2
* thunderbird
* timeshift
* tomboy
* transgui-2
* trine (Source Numix Circle)
* trine2 (Source Numix Circle)
* ubuntu-tweak
* uget
* unetbootin
* unity3d
* urbanterror
* utilities-system-monitor
* vdrift
* venom
* viber
* virtualbox
* vkaudiosaver
* vlc
* vmplayer
* warthunder
* wine
* winecfg
* wine-folder
* wine-help
* winetricks
* wine-uninstaller
* wolfram-mathematica
* wps-office-etmain
* wps-office-wppmain
* wps-office-wpsmain
* xbmc
* xournal
* yandex-browser-beta
* youtube
* assaultcube
* dftd_icon
* flightgear
* hon
* redeclipse
* sauerbraten
* srb2
* teewords
* worldofgoo
* xonotic
* torchlight2
* avidemux
* cantata
* crashplan (Source Numix Circle)
* curelew
* jitsi (Source Numix Circle)
* xnoise (Source Numix Circle)
* xnview (Source Numix Circle & elementary)
* viewnior
* tomahawk
For panel
* alarm-clock
* alarm-clock-triggered
* deluge
* diodon-panel
* indicator-rss-aware
* indicator-rss-aware-attention
* indicator-sound-switcher
* indicator-stickynotes
* laptop-symbolic
* mega
* network-idle
* psensor_hot
* psensor_normal
* simplescreenrecorder
* simplescreenrecorder-paused
* simplescreenrecorder-recording
* smartphone-symbolic
* steam_tray_mono
* synapse
* tablet-symbolic
* transmission-panel
* shutter
* qubittorrent
* clementine-panel
* clementine-panel-grey
* deadbeef
* tomahawk
extra
my-weather-indicator (Source elementary)
* mwil-chance-of-rain and other...
raiotray
* radiotray_connecting
* radiotray_off
* radiotray_on
telegram
* ico_22_1
* icomute_22_1
pomodoro-indicator
* pomodoro-indicator-all and other...
actions
* view-refresh
* go-home
* media-removable
* media-playlist-repeat
* media-playlist-shuffle
* AND MORE OTHERS

* **Dropbox Status Icons:** Manuel Kehl's adaption of the Dropbox status icons of the [Numix Circle](https://github.com/numixproject/numix-icon-theme-circle) icon set
* **Original Elementary Icons:** Several icons from the original elementary icon theme

Aneurin Hall (aneurin.hall@gmail.com)
Application icons
* Tux Guitar

Micah Ilbery (TraumaD91)
* Various LibreOffice icons
* Tweetdeck
* Pithos
* Nuvola Player
* Dolphin Emu
* Various Steam games icons

Caleb Riley (spiceofdesign)
* Spotify application icon

Samuell Hewitt (hewittsamuel)
* Minecraft

Amathadius (amathadius.deviantart.com)
* Transmission
* Chromium

Yuly Nesterov
* Design idea for icons simplescreenrecorder to tray

Andrea Soragna (sora-meliae.deviantart.com)
* Unetbootin
* winusbgui-icon
